...
function fetchLangData(lang) {
	...
	return fetch(`${docRoot}i18n/${lang}.json`)
		...
		.then(json => {
			i18n = json;
			// skillData = i18n.skillData;
			skillData = setupSkillData(lang);//i18n.skillData;
			return i18n;
		});
}
...
var skillData;// = i18nData.skillData;
var skillDataFlatList;
...
function setupSkillData() {
	...
	const makeSkill=(id, iKey, arg1, options) => {
		const skill = { id, name:i18n[`${iKey}_T`], desc:desc(iKey, arg1), ...options };
		skillDataFlatList.push(skill);
		return skill;
	};
...
function translateNonSkillData() {
	...
	$("#popup-window .indent").html( i18n["copy_hotkey"].replace("$1", `<span class="hotkey">Ctrl-C</span>`) );
...
function _openLangSelect() {
	...
	const langToBtn = ([code,name])=>`<button class='lang-choose-button' data-lang='${code}'><img src='images/flags/${code}.png' /> ${name}</button>`;
	var columns = [], tLangArray = Object.entries(LANGUAGES);
	for(let i = 0; i < Math.ceil(tLangArray.length/6); i++) {
		var langs = tLangArray.slice(i*6, (i+1)*6-1); // Split into 6 langs per column
		var btns = langs.map(langToBtn).join("");
		columns.push(`<div class='server-button-column'>${btns}</div>`);
	}
	html += columns.join("");
	
	$.featherlight($("<div/>", { html:html }), { otherClose:".lang-choose-button" });
	
	$(".lang-choose-button").on("click", function(){
